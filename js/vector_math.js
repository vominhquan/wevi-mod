...
function backpropagate(inputVectors, outputVectors, inputNeurons, hiddenNeurons, outputNeurons, expectedOutput) {
  ...
  C = expectedOutput.reduce(function (a, b) {
    return a + b
  }, 0)

  var errors = [];
  outputNeurons.forEach(function(n, i) {
    error_i = n['value']*C - expectedOutput[i]
    errors.push(error_i);
  ...
  outputVectors.forEach(function(v, i) {  // i: vocab index (opposite to my paper's notations)
    v.forEach(function(e, j) {  // j: hidden layer index
      ...
      console.log(errors[i].toFixed(3)+" " +hiddenNeurons[j]['value']);
  ...
  console.log("EI");
  errors.forEach((x)=> console.log(x.toFixed(3)));
  console.log("EH");
  hiddenNeurons.forEach(function(x) {console.log(x.net_input_gradient.toFixed(3))})
  console.log("")
  /////// chỉ cbow mới quan tâm
  var numInputExcited = 0;
  ...
  inputNeurons.forEach(function(n,i) {
    if (n['value'] < 1e-5) {  // should be either 0 or 1
      ...
      numInputExcited += 1; //////// số lượng input
    }
  });
